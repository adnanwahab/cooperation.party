...
city_locations = {
    "San-Diego--California--United-States.json": [32.715736, -117.161087],
    "Austin--Texas--United-States.json": [30.267153, -97.743057],
    "Boston--Massachusetts--United-States.json": [42.360256, -71.058882],
    "Denver--Colorado--United-States.json": [39.739235, -104.990250],
    "Chicago--Illinois--United-States.json": [41.878113, -87.629799],
    "Washington--D.C.--District-of-Columbia--United-States.json": [38.907192, -77.036873],
    "New-Orleans--Louisiana--United-States.json": [29.951065, -90.071533]
}
...
def compute_deal_ranking(apt_json):
    for city in apt_json:
        ...
        for key in apt_json[city]:
            ...
            h3_index = h3.geo_to_h3(float(apt['latitude']), float(apt['longitude']), 6)
...
def compute_travel_time(apt_json, places):
    for city in apt_json:
        places = fetch_coworking(city_locations[city])
        for apt_id in apt_json[city]:
            apt = apt_json[city][apt_id]
            nearest = getNearest(places, apt)
            apt['commute_distance'] = distanceToPoi(nearest, apt)
    return apt_json
...
def filter_columns(list_of_items, important_columns):
    ...
    for apt_id in list_of_items:
        ...
        result[apt_id] = second_item
    #print(result)
    return result
    
    
def rankAptByUserPreferences():
...
#      'New-Orleans--Louisiana--United-States.json',
    ...
    apt_json = {
        city: filter_columns(json.load(open(f'./data/columns/{city}')), important_columns) for city in special_case#cities[:1])
    }

    apt_json, h3_prices = compute_deal_ranking(apt_json)
    #apt_json, h3_complaints = compute_311(apt_json)
    #print(h3_complaints)
    apt_json = compute_travel_time(apt_json, ['library', 'yoga studio'])
    return apt_json
...
def find_best_deals_on_airbnb(prevData, documentContext, sentence):
    ...
    return {'data': rankAptByUserPreferences(),
            'component':'<tableview>',
            'cities': ['San-Diego--California--United-States.json',
                        'Austin--Texas--United-States.json',
                        'Boston--Massachusetts--United-States.json',
                        'Denver--Colorado--United-States.json',
                        'Chicago--Illinois--United-States.json',
